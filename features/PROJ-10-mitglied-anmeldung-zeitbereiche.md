...
- [ ] Angenommen die Übersicht-Seite wird geöffnet, dann zeigt sie pro Zeitbereich Zeitbereich-Label, kommen, benötigt (insg.), offen (benötigt − kommen) sowie das passende Status-Icon — ohne Namen
...
| Anmeldung-Seite zeigt volle Namen (Nachname Vorname) aller bereits zugesagten Mitglieder pro Zeitbereich; Übersicht-Seite zeigt nur aggregierte Zahlen ohne Namen | Nutzerentscheidung im Interview; Übersicht bleibt bewusst datensparsam (passend zum Adalo-Datenleck-Vorfall, der SimpliPlan ausgelöst hat), während die Anmeldung-Seite wie in der alten Adalo-Referenz volle Transparenz bietet, wer mit wem eingeteilt ist | 2026-07-15 |
...
└── Tabelle je Zeitbereich (nur benötigt > 0, Anlage-Reihenfolge):
    ...
    Status-Icon (zu wenig / genau richtig / zu viel) — keine Namen
